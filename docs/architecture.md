...
**Зачем нужен `src/bot/bot.ts`:** это единственная точка создания бота.
Файл собирает grammY-инстанс: подключает сессию, регистрирует все обработчики
в правильном порядке и кэширует результат, чтобы route handler мог просто
вызвать `getBot()` и не пересоздавать бота на каждое обновление Telegram.
